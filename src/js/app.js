...
const showTooltip = (el) => {
    actualMessages.push({
        name: el.name,
        id: tooltipFactory.showTooltip("And here's some amazing content. it`s very engaging. right?", el)
    })
}

btn.addEventListener('click', (e) => {
    e.preventDefault();
    const currentErrorMessage = actualMessages.find(item => item.name === btn.name);
